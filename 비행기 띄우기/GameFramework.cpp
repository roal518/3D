...
bool CGameFramework::OnCreate(HINSTANCE hInstance, HWND hMainWnd)
{
	...
	// Direct3d 디바이스,명령 큐와 명령 리스트 ,  스왑체인등을 생성하는 함수를 호출한다.
...
void CGameFramework::CreateRtvAndDsvDescriptorHeaps()
{
	...
	//랜더 타겟 서술자 힙(서술자의 개수는 스왑체인 버퍼의 개수)를 생성한다.
...
void CGameFramework::CreateDirect3DDevice()
{
	...
	for (int i = 0; i < 2; ++i)m_nFenceValues[i] = 0;
...
void CGameFramework::BuildObjects()
{
	...
	CAirplanePlayer* pAirplanePlayer = new CAirplanePlayer(m_pd3dDevice,
		m_pd3dCommandList, m_pScene->GetGraphicsRootSignature());//여기서 웬만한거 다 받아주는데?
