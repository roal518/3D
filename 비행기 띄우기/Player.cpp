...
ID3D12RootSignature* CPlayer::GetGraphicsRootSignature()
{
	return(m_pd3dGraphicsRootSignature);
}
ID3D12RootSignature* CPlayer::CreateGraphicsRootSignature(ID3D12Device* pd3dDevice)
{
...
{
	OutputDebugString(TEXT("-----------PLAYER RENDER-----------------\n"));
	...
	* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	...
	CMesh* pAirplaneMesh = new CAirplaneMeshDiffused(pd3dDevice, pd3dCommandList, 20.0f,
		20.0f, 4.0f, XMFLOAT4(0.0f, 0.5f, 0.0f, 0.0f)); //컬러까지 설정

	SetMesh(pAirplaneMesh);
	//플레이어의 카메라를 스페이스-쉽 카메라로 변경(생성)한다. 
	m_pCamera = ChangeCamera(THIRD_PERSON_CAMERA, 0.0f); //카메라는 여기서 바꾼다..
	//플레이어의 위치를 설정한다. 
	SetPosition(XMFLOAT3(0.0f, 0.0f, -50.0f));
	
	//플레이어를 위한 셰이더 변수를 생성한다. 
	CreateShaderVariables(pd3dDevice, pd3dCommandList);//여기까지는 같다
	...
	SetShader(pShader);
	assert(m_pShader != nullptr && " error ");
	m_pShader->CheckSum();
	OutputDebugString(TEXT("-----------PLAYER OUT-----------------\n"));
