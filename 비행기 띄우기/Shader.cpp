...
D3D12_RASTERIZER_DESC CShader::CreateRasterizerState()
{
	...
	d3dRasterizerDesc.FillMode = D3D12_FILL_MODE_SOLID;//WIREFRAME-SOLID
	d3dRasterizerDesc.CullMode = D3D12_CULL_MODE_BACK;//None-Back-Front 가 있다.
...
void CShader::OnPrepareRender(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	//assert(m_ppd3dPipelineStates != nullptr && "Pipeline states array is null.");
	//assert(m_ppd3dPipelineStates[0] != nullptr && "Pipeline state object is null.");
	if(m_ppd3dPipelineStates)pd3dCommandList->SetPipelineState(m_ppd3dPipelineStates[0]);
}

void CShader::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	OutputDebugString(TEXT("-----------WORKSTATION IN-----------------\n"));
	assert(pd3dCommandList != nullptr && "pd3dCommandList is null.");
	assert(pCamera != nullptr && " is null.");
	OnPrepareRender(pd3dCommandList);
	OutputDebugString(TEXT("-----------WORKSTATION OUT-----------------\n"));
}
	...
	* pd3dCommandList)
{
	...
	12.0f, 12.0f, 12.0f);
	CAirplaneMeshDiffused* pAirplaneMesh = new CAirplaneMeshDiffused(pd3dDevice, pd3dCommandList, 20.0f,
	...
	for (int x = -xObjects; x <= xObjects; x++)
	{
		for (int y = -yObjects; y <= yObjects; y++)
		{
			for (int z = -zObjects; z <= zObjects; z++)
			{
				...
				pRotatingObject->SetMesh(pAirplaneMesh);
				//각 정육면체 객체의 위치를 설정한다. 
